    ...
    fn receive_and_buffer_packets(
        ...
        match decision {
            BufferedPacketsDecision::Consume(_) | BufferedPacketsDecision::Hold => loop {
                if start.elapsed() > MAX_RECV_TIME || result >= MAX_PER_RECV {
                    return Ok(result);
                }
                ...
                let (parse_result, duration_us) = measure_us!(Self::parse_batch(
                    &batch,
                    &self.bank_forks,
...
mod tests {
    ...
    fn test_parse_bundle_success() {
        ...
        let result = BamReceiveAndBuffer::parse_batch(
            &bundle,
            &bank_forks,
            &HashSet::new(),
    ...
    fn test_deserialize_packets_invalid_signature() {
        ...
        let result = BamReceiveAndBuffer::deserialize_packets(
            packets.iter(),
            &mut BamReceiveAndBufferMetrics::default(),
        );
        assert!(result.is_err());
